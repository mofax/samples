<script lang="ts">
	...
	async function postDataLoginData(body: Dict) {
		const response = await fetch('/login/password', {
			method: 'POST',
			body: JSON.stringify({ action: 'login', payload: body }),
			headers: {
	...
	{#if supported === true}
		...
		<form
			...
			on:submit={e => {
				...
				const form = extractFormValues(formData, 'email', 'password');
			...
			<div class="button">
				<div class="flex gap-3">
					<Button
						...
						variant="brand"
						>Login
						<Fingerprint style={'margin-left:auto'} />
					</Button>
