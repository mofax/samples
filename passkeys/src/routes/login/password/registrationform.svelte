<script lang="ts">
	...
	async function postRegistrationData(body: Dict) {
		const res = await fetch('/login/password', {
			method: 'POST',
			body: JSON.stringify({ action: 'register', payload: body }),
			headers: {
		...
		if (res.ok) {
			createToast('An account was created, you can now login', {
				duration: 5000,
			});
			setTimeout(() => {
				toggle();
...
<div class="wrapper">
	...
	{#if supported === true}
		<h2>Enter an email and password</h2>
		...
		<form
			...
			on:submit={async e => {
				...
				if (form.email !== form.confirmEmail) {
					formError = "Emails don't match";
					return;
				}
				await postRegistrationData(form);
				submitting = false;
			}}
		>
			...
			<div class="button">
				...
				<br />
				<button class="btn-link" on:click={() => toggle()}
					>Login to existing account</button
				>
				<br />
				<div class="flex gap-5">
					<a href="/login">Use passkeys instead</a>
				</div>
			</div>
		</form>
	{/if}
