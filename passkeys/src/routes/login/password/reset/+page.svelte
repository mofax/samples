<script lang="ts">
	...
	if (import.meta.env.DEV) {
		formData = {
			...
			confirmEmail: 'mila.alloys@gmail.com'
		};
	}

	async function postDataLoginData(body: Dict) {
		submitting = true;
		...
		if (response.ok) {
			successMessage = "A code has been sent to the email you provided"
		} else {
...
<div class="wrapper">
	...
	{#if supported === true}
		<h2>Enter the email on your akiza account</h2>
		<p>We will send a reset code to that email address</p>
		...
		<form
			...
			on:submit={(e) => {
				...
				const form = extractFormValues(formData, 'email');
				postDataLoginData(form);
			}}
		>
			...
			<div class="button">
				<div class="flex gap-3">
					<Button disabled={submitting} busy={submitting} type="submit" variant="brand"
						>Send reset code
					</Button>
				</div>
				<br />
